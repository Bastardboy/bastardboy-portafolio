<template>
  <section class="flex flex-col md:flex-row items-center gap-8 w-full max-w-7xl mx-auto p-6 min-h-[70vh]">
    <!-- Contenedor de Imagen (Primero en móvil) -->
    <div class="md:w-1/3 order-1 md:order-2 flex justify-center items-center lg:mt-20">
      <transition
        mode="out-in"
    ...
    <!-- Contenedor de Información (Segundo en móvil) -->
    <div class="md:flex-1 order-2 md:order-1 w-full max-w-prose mx-auto flex flex-col justify-center">
      <!-- Nombre Centrado -->
      ...
      <ul v-if="aboutMe.info" class="space-y-4">
        <li 
          v-for="(desc, index) in aboutMe.info"
          :key="index"
          class="flex items-center p-4 rounded-lg shadow-sm hover:shadow-md transition-all"
        ...
        >
          ...
          >
            <div :key="desc" class="flex items-center">
